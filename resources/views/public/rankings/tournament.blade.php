...
<div class="pub-wrap">
    ...
    <div class="pub-actions">
        ...
        @php
            // Active ranking systems this tournament feeds (if the relation is loaded/available).
            $pubRankings = method_exists($tournament, 'rankingSystems')
                ? $tournament->rankingSystems->where('is_active', true)
                : collect();
        @endphp
        @foreach($pubRankings as $rs)
            <a href="{{ route('public.rankings.show', [$rs, 'tournament' => $tournament->id]) }}" class="pub-btn">
                <i class="fa-solid fa-ranking-star"></i> Ranking{{ $pubRankings->count() > 1 ? ': '.$rs->name : '' }}
            </a>
        @endforeach
